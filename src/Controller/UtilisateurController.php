...
class UtilisateurController extends AbstractController
{
    ...
    public function getClientUtilisateurs(int $id, UtilisateurRepository $utilisateurRepository, Request $request): JsonResponse
    {
        $page = $request->get('page', 1);
        $limit = $request->get('limit', 3);

        // Mise en cache de cette requete 
        $reponse = $this->cache->get("getUserByClient-".$id."Page-".$page."Limit-".$limit, function() use ($utilisateurRepository, $id, $page, $limit){
            $utilisateurs = $utilisateurRepository->findUtilisateursByClientIdAndPagination($id, $page, $limit);
    ...
    public function ajoutUtilisateur(int $idClient, Request $request , EntityManagerInterface $entityManager, SerializerInterface $serializer, UserPasswordHasherInterface $passwordHasher, ClientRepository $clientRepository): JsonResponse{
        ...
        try{
            ...
            return $this->json($utilisateur, 201, [], ['groups' => 'client'] );
        }
    ...
    public function suppressionUtilisateur(int $idClient, int $idUtilisateur, EntityManagerInterface $entityManager, UtilisateurRepository $utilisateurRepository): JsonResponse {
        try {
            ...
            if ($utilisateur->getClient()->getId() != $idClient) {
                return $this->json(['status' => 421, 'message' => "L'utilisateur n'appartient pas à ce client."], 421);
            }
            ...
            return $this->json(['status' => 204, 'message' => "L'utilisateur a bien été supprimé."], 204);
